...
- Sử dụng lệnh `/ck:git cm` (commit)
- Sử dụng lệnh `/ck:git cp` (commit và push)
- Sử dụng lệnh `/ck:git pr` (tạo pull request)
...
- **Kiểm tra trước khi đẩy**: Sử dụng `/ck:git cm` để xem thông điệp trước khi dùng `/ck:git cp`.
