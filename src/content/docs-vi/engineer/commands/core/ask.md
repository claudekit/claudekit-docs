...
```bash
/ck:ask [nên sử dụng kiến trúc microservices hay monolithic cho một nền tảng SaaS với 10.000 người dùng?]
```
...
```bash
/ck:ask [chúng tôi cần thêm tính năng theo dõi phân tích hành vi người dùng. Nên sử dụng PostgreSQL với TimescaleDB, ClickHouse, hay một dịch vụ phân tích riêng biệt như Mixpanel?]
```
...
```
- Nếu bạn cần /ck:scout để tìm các cấu hình DB hiện có, tôi có thể gọi lệnh đó
...
```bash
/ck:ask [lựa chọn giữa Next.js, Remix và Astro cho trang web tài liệu có tính năng chat AI]
```
...
```bash
/ck:ask [thời gian phản hồi API của chúng tôi tăng lên 2 giây khi tải nặng. Nên thêm bộ nhớ đệm, mở rộng hàng ngang hay tối ưu hóa truy vấn?]
```
...
```bash
/ck:ask [chúng tôi nên tích hợp xử lý thanh toán như thế nào: tích hợp trực tiếp Stripe hay lớp trừu tượng cổng thanh toán (payment gateway abstraction layer)?]
```
...
```bash
/ck:ask [cách tiếp cận tốt nhất cho xác thực API: JWT, dựa trên session, hay API keys?]
```
...
✅ **Tốt:**
```bash
/ck:ask [nên sử dụng WebSockets hay Server-Sent Events cho thông báo thời gian thực?]
/ck:ask [cách cấu trúc ranh giới microservices cho lĩnh vực thương mại điện tử?]
/ck:ask [kiến trúc cơ sở dữ liệu nào cho SaaS đa khách hàng (multi-tenant) có cách ly dữ liệu?]
```

❌ **Quá tập trung vào Triển khai:**
```bash
/ck:ask [làm thế nào để viết một hàm kết nối với Redis?]
/ck:ask [cú pháp cho các chỉ mục (indexes) PostgreSQL là gì?]
/ck:ask [debug thông báo lỗi này giúp tôi]
```
...
Lệnh `/ck:ask` tự động đọc thư mục `./docs` nhưng bạn có thể hỗ trợ bằng cách:
...
## Điều /ck:ask KHÔNG làm

- ❌ Viết mã nguồn triển khai
- ❌ Sửa lỗi (sử dụng `/ck:debug` hoặc `/ck:fix` thay thế)
- ❌ Triển khai hạ tầng
...
/ck:ask [cách tiếp cận tốt nhất cho xử lý công việc chạy ngầm (background job)?]

# 2. Xem xét các đề xuất
# [Nhận kết quả từ cố vấn]

# 3. Tạo kế hoạch triển khai
/ck:plan [triển khai background jobs sử dụng Bull + Redis]

# 4. Triển khai
/ck:cook [thêm xử lý background job]
```
...
/ck:ask [liệu sự trừu tượng hóa lớp dịch vụ (service layer) này có quá phức tạp cho các hoạt động CRUD không?]

# 3. Điều chỉnh dựa trên hướng dẫn
# [Thực hiện các thay đổi nếu được đề xuất]
```

### Có thể gọi /ck:scout tự động

Nếu `/ck:ask` cần thêm ngữ cảnh về mã nguồn của bạn:

```
Phân tích Kiến trúc:
Cần hiểu thiết lập cơ sở dữ liệu hiện tại...

Đang gọi /ck:scout để tìm cấu hình cơ sở dữ liệu...
...
/ck:scout [tìm tất cả các triển khai xác thực API] 3

# Sau đó đặt câu hỏi kiến trúc
/ck:ask [nên hợp nhất các mẫu xác thực này hay giữ chúng riêng biệt?]
```
...
```bash
/ck:ask [cách tốt nhất để xử lý tải lên tệp trên 100MB với tính năng theo dõi tiến trình và khả năng tiếp tục khi bị gián đoạn là gì?]
```
...
```bash
/ck:ask [cách tiếp cận xác thực cho ứng dụng di động với yêu cầu: ưu tiên ngoại tuyến, đồng bộ hóa khi có mạng]
```
...
- Sửa lỗi (Bugs): `/ck:debug [vấn đề]`
- Triển khai: `/ck:plan` sau đó `/ck:cook`

## Các lệnh liên quan

- [/ck:scout](/docs/engineer/skills/scout) - Tìm kiếm mã nguồn trước khi đặt câu hỏi kiến trúc
- [/ck:plan](/docs/engineer/commands/plan) - Tạo kế hoạch triển khai sau khi nhận được hướng dẫn
- [/ck:debug](/docs/engineer/commands/core/debug) - Điều tra các vấn đề kỹ thuật (không phải kiến trúc)

---

**Thông điệp chính**: `/ck:ask` cung cấp tư vấn kiến trúc chiến lược từ các cố vấn chuyên gia, giúp bạn đưa ra các quyết định kỹ thuật sáng suốt trước khi bắt đầu triển khai.
