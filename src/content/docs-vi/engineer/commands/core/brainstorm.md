...
```bash
/ck:brainstorm [nên sử dụng Redis hay PostgreSQL để lưu trữ session?]
```
...
```bash
/ck:brainstorm [nên sử dụng quản lý trạng thái nào cho bảng điều khiển React có cập nhật thời gian thực?]
```
...
```bash
/ck:brainstorm [monorepo với Turborepo hay các repo riêng biệt cho 5 dịch vụ?]
```
...
```bash
/ck:brainstorm [Prisma vs Drizzle vs SQL thuần cho API Node.js mới?]
```
...
```bash
/ck:brainstorm [cách xử lý tải lên tệp: trực tiếp lên S3 hay thông qua máy chủ API?]
```
...
```bash
/ck:brainstorm [chuyển đổi dần dần từ Express sang Fastify hay viết lại toàn bộ?]
```
Nhận phân tích rủi ro và đề xuất cách tiếp cận theo từng giai đoạn.

## Điều /ck:brainstorm KHÔNG làm

- ❌ Triển khai mã nguồn (sử dụng `/ck:cook`)
- ❌ Sửa lỗi (sử dụng `/ck:debug` hoặc `/ck:fix`)
...
✅ **Tốt**:
```bash
/ck:brainstorm [PostgreSQL vs MongoDB cho nội dung do người dùng tạo với các bình luận lồng nhau?]
/ck:brainstorm [SSR vs SSG vs ISR cho trang web tài liệu có cập nhật hàng ngày?]
```

❌ **Quá mơ hồ**:
```bash
/ck:brainstorm [tôi nên dùng cơ sở dữ liệu nào?]
/ck:brainstorm [làm thế nào để xây dựng cái này?]
```
...
/ck:brainstorm [chiến lược xác thực tốt nhất cho SaaS đa khách hàng?]

# 2. Xem báo cáo, thảo luận với đội ngũ
cat plans/reports/brainstorm-251129-auth-strategy.md

# 3. Tạo kế hoạch triển khai
/ck:plan [triển khai xác thực JWT với sự cách ly giữa các khách hàng]
...
/ck:brainstorm [liệu sự trừu tượng hóa dịch vụ này có đáng với độ phức tạp của nó không?]
...
- [/ck:ask](/docs/engineer/commands/core/ask) - Các câu hỏi kiến trúc nhanh không cần báo cáo đầy đủ
- [/ck:plan](/vi/docs/engineer/commands/plan) - Tạo kế hoạch triển khai sau khi brainstorm
- [/ck:cook](/docs/engineer/skills/cook) - Thực thi kế hoạch với các bước kiểm soát chất lượng
- [/ck:cook](/docs/engineer/skills/cook) - Triển khai tính năng theo từng bước

---

**Thông điệp chính**: `/ck:brainstorm` cung cấp phân tích đa chiều có cấu trúc cho các quyết định kỹ thuật, ghi lại các đề xuất dựa trên các nguyên tắc YAGNI, KISS và DRY. Hệ thống tư vấn - bạn là người quyết định.
