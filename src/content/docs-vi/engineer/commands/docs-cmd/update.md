...
```bash
/ck:docs:update [yêu cầu bổ sung]
```
...
Lệnh `/ck:docs:update` sử dụng agent `docs-manager` với quy trình làm việc sau:
...
/ck:docs:update [tập trung vào các thay đổi kiến trúc API]
```

**Thành Viên Mới Trong Nhóm**
```bash
# Chuẩn bị tài liệu để hướng dẫn thành viên mới
/ck:docs:update [đảm bảo tất cả hướng dẫn thiết lập đều mới nhất]
```

**Trước Khi Phát Hành**
```bash
# Trước khi phát hành phiên bản mới
/ck:docs:update [chuẩn bị cho việc phát hành phiên bản v2.0]
```

**Bảo Trì Định Kỳ Hàng Quý**
```bash
# Xem lại tài liệu định kỳ
/ck:docs:update [kiểm tra tài liệu hàng quý]
```
...
```bash
❌ /ck:docs:update [sửa lỗi đánh máy trong README]
✅ Chỉ cần chỉnh sửa trực tiếp tệp README
```

**Không Có Thay Đổi Mã Nguồn**
```bash
❌ /ck:docs:update [chỉ kiểm tra thử]
✅ Chỉ chạy sau khi có những thay đổi mã nguồn đáng kể
```

**Kiểm Tra Trạng Thái Nhanh**
```bash
❌ /ck:docs:update [có gì thay đổi?]
✅ /ck:watzup [xem lại các thay đổi gần đây]
```
...
```bash
/ck:docs:update [cập nhật tài liệu API với các endpoint mới]
```
...
```bash
/ck:docs:update [chuẩn bị tài liệu cho phiên bản phát hành v2.0]
```
...
```bash
/ck:docs:update [tập trung vào kiến trúc hệ thống]
```
...
```bash
# Sau khi triển khai tính năng
/ck:cook [thêm tích hợp thanh toán]
/ck:fix
/ck:test
/ck:docs:update
...
✅ **Tập trung cụ thể:**
```bash
/ck:docs:update [tập trung vào các thay đổi API và luồng xác thực]
```

❌ **Không có ngữ cảnh:**
```bash
/ck:docs:update  # Vẫn hoạt động nhưng kém mục tiêu hơn
```
...
/ck:plan [thêm xác thực OAuth2]

# 2. Triển khai tính năng
/ck:cook [triển khai OAuth2 với các nhà cung cấp Google và GitHub]
...
# 4. Cập nhật tài liệu
/ck:docs:update [tài liệu hóa việc triển khai OAuth2]
...
```bash
# 1. Cập nhật tài liệu
/ck:docs:update [xem lại tài liệu hàng quý]
...
```bash
# 1. Cập nhật tài liệu
/ck:docs:update [chuẩn bị cho phiên bản phát hành v2.0]
...
/ck:docs:update [cập nhật system-architecture.md và deployment-guide.md]
```
...
/ck:docs:update [làm mới tất cả các ví dụ mã]
```
...
/ck:docs:update [tài liệu hóa dịch vụ thanh toán mới]
```
...
Sau khi chạy `/ck:docs:update`, cấu trúc tài liệu của bạn sẽ như sau:
...
Hiệu suất điển hình của `/ck:docs:update`:
...
Sau khi sử dụng `/ck:docs:update`:

- [/ck:docs:summarize](/vi/docs/engineer/commands/docs-cmd/summarize) - Chỉ cập nhật bản tóm tắt codebase
- [/ck:watzup](/docs/engineer/commands/core/watzup) - Xem lại các thay đổi gần đây
- [/ck:git cm](/docs/engineer/skills/git) - Commit các cập nhật tài liệu

---

**Điểm mấu chốt**: `/ck:docs:update` đảm bảo tài liệu của bạn luôn đồng bộ với codebase thông qua phân tích toàn diện và cập nhật hệ thống trên tất cả các tệp tài liệu.
