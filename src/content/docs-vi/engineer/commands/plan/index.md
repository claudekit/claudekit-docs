...
Bộ định tuyến lập kế hoạch thông minh. Phân tích độ phức tạp tác vụ, đặt câu hỏi làm rõ nếu cần, và định tuyến đến quy trình lập kế hoạch phù hợp (`/ck:plan:fast` hoặc `/ck:plan:hard`).
...
**Kết quả**: Tác vụ phức tạp được định tuyến đến `/ck:plan:hard` với prompt đã nâng cao.
...
Bước 5: Nâng cao prompt với ngữ cảnh
        ↓
Bước 6: Thực thi /ck:plan:fast hoặc /ck:plan:hard
        ↓
...
Executing /ck:plan:hard with enhanced prompt...
Activating planning skill...

[/ck:plan:hard creates detailed implementation plan]
...
/ck:plan [implement real-time notifications system]
/ck:plan [add multi-tenant support to the platform]
/ck:plan [migrate from REST to GraphQL]
/ck:plan [implement end-to-end encryption for messages]
```
...
|------|-------|-----------------|
| [/ck:plan --fast](/docs/engineer/skills/plan) | Lập kế hoạch nhanh không nghiên cứu | Tác vụ đơn giản, rõ ràng |
| [/ck:plan --hard](/docs/engineer/skills/plan) | Lập kế hoạch chi tiết có nghiên cứu | Tác vụ phức tạp |
| [/ck:plan --parallel](/docs/engineer/skills/plan) | Kế hoạch với giai đoạn thực thi song song | Thực thi đa agent |
| [/ck:plan --two](/docs/engineer/skills/plan) | So sánh hai cách triển khai | Quyết định kiến trúc |
...
Để `/ck:plan` quyết định độ phức tạp:
...
/ck:plan:hard [add caching layer]  # Có thể quá mức cần thiết
```
...
**Vấn đề**: Hầu hết tác vụ được định tuyến đến `/ck:plan:hard`
...
**Điểm chính**: `/ck:plan` là điểm vào lập kế hoạch thông minh của bạn. Nó phân tích độ phức tạp, đặt câu hỏi đúng, và định tuyến đến quy trình lập kế hoạch phù hợp - để bạn có đủ kế hoạch cho mỗi tác vụ.
