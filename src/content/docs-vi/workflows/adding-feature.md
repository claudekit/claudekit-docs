...
**Các lệnh**: /ck:plan, /ck:cook, /ck:test, /ck:docs update, /ck:git cm
...
```bash
/ck:plan [thêm luồng đặt lại mật khẩu với xác minh email]
```
...
```bash
/ck:scout "Cho tôi xem mã nguồn xác thực hiện có" 3
```
...
```bash
# Bộ review mã nguồn chạy tự động sau lệnh /ck:cook
# Hoặc gọi thủ công
/review
```
...
```bash
/ck:plan [thêm tìm kiếm sản phẩm với bộ lọc, sắp xếp và phân trang]
```
...
```bash
/ck:cook [triển khai tìm kiếm sản phẩm theo kế hoạch]
```
...
/ck:cook [thêm endpoint GET /api/users/:id/profile]
```
...
/ck:plan [triển khai kiến trúc đa khách thuê (multi-tenant) với sự cô lập khách thuê]
/ck:cook [triển khai kiến trúc đa khách thuê]
```
...
/ck:cook [triển khai backend API cho thông báo]
/ck:cook [triển khai bảng thông báo ở frontend]
```
...
/ck:plan [tích hợp thông báo SMS Twilio]
/ck:cook [tích hợp Twilio SMS theo kế hoạch]
```
...
/ck:plan [thêm quản lý người dùng - giai đoạn 1: CRUD người dùng]
/ck:cook [triển khai CRUD người dùng]

/ck:plan [thêm quản lý người dùng - giai đoạn 2: vai trò và quyền hạn]
/ck:cook [triển khai vai trò và quyền hạn]
```
...
/ck:cook [thêm xử lý lỗi cho sự cố mạng trong luồng đặt lại mật khẩu]
```
...
```bash
# Thêm tối ưu hóa
/ck:cook [tối ưu hóa các truy vấn tìm kiếm với index DB và bộ nhớ đệm]
```
...
/ck:docs update [tập trung vào luồng đặt lại mật khẩu kèm sơ đồ]
```
...
```bash
✅ Tốt:
/ck:cook [thêm chức năng tải ảnh đại diện người dùng]
/ck:cook [thêm chức năng tạo ảnh thu nhỏ (thumbnail)]

❌ Quá lớn:
/ck:cook [thêm hệ thống quản lý phương tiện hoàn chỉnh]
```
...
```bash
/ck:cook [tính năng]
/ck:test           # Luôn chạy kiểm thử
/ck:fix       # Sửa lỗi ngay lập tức
```
...
```bash
/ck:cook [tính năng]
/ck:docs update    # Cập nhật tài liệu ngay
```
...
- [/ck:git cm](/docs/engineer/skills/git) - Commit thay đổi

### Đọc thêm
- [Tham khảo skills](/vi/docs/engineer/skills) - Tất cả kỹ năng có sẵn
